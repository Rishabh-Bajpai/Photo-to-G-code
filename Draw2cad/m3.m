...
% Last Modified by GUIDE v2.5 20-Apr-2019 22:15:19
...
for i=1:length(X_path)-1
    if(abs(X_path(i)-X_path(i+1))>20)
        ...
        i
        try
        thx=fit(t(first_temp:last_temp)',X_path(first_temp:last_temp)','cubicspline');
        X_fit{x_i}=thx;
...
for i=1:length(Y_path)-1
    if(abs(Y_path(i)-Y_path(i+1))>20)
        ...
        i
        try
        thy=fit(t(first_temp:last_temp)',Y_path(first_temp:last_temp)','cubicspline');
        Y_fit{y_i}=thy;
